"""The CarliniWagnerL2 attack from cleverhans package
"""
import numpy as np
import tensorflow as tf
from utils_attack import get_or_guess_labels, set_with_mask
import keras
...
# Load a sample MNIST image (assuming `mnist_model` is your trained model)
(x_train, y_train), (x_test, y_test) = tf.keras.datasets.mnist.load_data()

# Use the first image from the test set as an example
original_image = x_test[30]
original_image = original_image.astype(np.float32) / 255.0  # Normalize the image
original_image = np.expand_dims(original_image, axis=-1)  # Reshape to (28, 28, 1)

# Get the original label
original_label = y_test[0]

# Convert the original label to one-hot encoding
target_class_ohe = to_categorical(original_label, num_classes=10)


model_path = "/ssd-sata1/mwt/def_project/DiffRobOT/DiffRobOT/MNIST/LeNet5_MNIST_logits.h5"
# model_path = "/ssd-sata1/mwt/def_project/DiffRobOT/DiffRobOT/attacks/ai_security/models/mnist_logits.h5"
mnist_model_logits = keras.models.load_model(model_path)
mnist_model_logits.summary()

# Define your model function (assuming logits output)
def model_fn(x):
    
    return mnist_model_logits(x)

# Instantiate the Carlini-Wagner L2 attack
cw_attack = CarliniWagnerL2(model_fn=model_fn, targeted=False)

# Perform the attack
perturbed_image = cw_attack.attack(np.expand_dims(original_image, axis=0))

# Calculate the total perturbation
total_perturbation = np.linalg.norm(perturbed_image - original_image)

# Get the original label prediction
# assuming normal output
mnist_model = keras.models.load_model('/ssd-sata1/mwt/def_project/DiffRobOT/DiffRobOT/MNIST/LeNet5_MNIST_normal.h5')
# mnist_model = keras.models.load_model('/ssd-sata1/mwt/def_project/DiffRobOT/DiffRobOT/attacks/ai_security/models/mnist.h5')
original_pred = mnist_model.predict(np.expand_dims(original_image, axis=0))
original_label_pred = np.argmax(original_pred)

# Get the adversarial label prediction
adv_pred = mnist_model.predict(perturbed_image)
adv_label_pred = np.argmax(adv_pred)

# Extract number of iterations from the attack (modify the CW attack code if necessary)
# Assuming `loop_i` holds the number of iterations (this is just a placeholder; you may need to modify the CW class)
number_iterations = cw_attack.max_iterations  # or loop_i if tracked inside the attack loop

# Output the results
print(f"Original Label: {original_label_pred}")
print(f"Adversarial Label: {adv_label_pred}")
print(f"Number of Iterations: {number_iterations}")
print(f"Total Perturbation: {total_perturbation}")
print(f"Perturbed Image: {perturbed_image.squeeze()}")  # Squeeze to remove extra dimensions