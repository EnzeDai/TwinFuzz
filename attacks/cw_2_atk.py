...
binary_search_step = 10  # Number of times to adjust the constant with binary search
...
init_const = 1e2       # The initial constant c to pick as a first guess

k = 40 # We set initial k value to be 40
boxmin = 0.0 # min range
boxmax = 255.0 # max range
num_labels = 10 # Total number of labels in the current model

# class Cw_attack:
#     def __init__(self, image, model, binary_search_step = 10, max_iterations = 1000, abort_early = True, learning_rate = 1e-2, initial_constant = 1e-3, confidence = 1e-3):
...
#         # Import the image for further process
#         image_array = np.array(image) # 28*28
...
#         I = I[0:num_labels]
#         target_label = I[0] # choose the highest probability as the original label
#         target_label = np.eye(num_labels)[target_label]


#         # nex we need to define the modifier:
...
#         pert_image = tf.tanh(modifier + timg) * boxmul + boxplus


#         # Start to initialize the parameters
#         session = tf.Session()
#         session.run(tf.global_variables_initializer())
#         tensorlist = [n.name for n in session.graph]


# Carlini Wagner Attack, L_2
class CwL2:
    def __init__(self, model, batch_size=1, confidence=conf, targeted=targeted, learning_rate=lr, binary_search_steps=binary_search_step, max_iterations=max_iterations, abort_early=abort_early, initial_const=init_const, boxmin=-0.5, boxmax=0.5):
